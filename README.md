...
| [surface-recon](./skills/surface-recon/) | 0.8.0 | candidate | Maps authorized targets across network, interactive, command, artifact, device, firmware, and runtime planes, with explicit acceptance proofs and consequence gates |
| [cli-build](./skills/cli-build/) | 0.12.0 | candidate | Builds a CLI an agent can operate and a human can supervise. Distribution target first, then the command surface, polished human output by default, and safety proportional to stakes. Delegates TypeScript primitive adoption to the canonical cligentic skill instead of carrying a copied catalog |
